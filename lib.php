...
function update($table, $champ, &$message, $blog = FALSE) {
    ...
    if(!is_array($champ) || empty($champ))
        ...
        $sql = 'SELECT '.implode(', ', $champ).' FROM '.$table.' WHERE ';
        if($blog)
            $sql.= 'blog_id = '.$blog_id.' ';
        else {
            $sql.= $sql_value.' ';
        ...
        if($rs = mysql_query($sql)) {
            ...
            while($row = mysql_fetch_assoc($rs)) {
                ...
                if(!do_update(sprintf($update, mysql_real_escape_string($row[$value]), $row[$id]))) {
                    $message['fatal'][] = sprintf(STR_ERROR_FATAL_CHAMP, $id, $row[$id], $value, $row[$value], $table);
                }
            }
        }
    }
...
/*
 * Fonction get_blogs_id()
 * -----
 * Retourne un tableau contenant les IDs de tous les blogs
 * -----
 * @return  array                        Les IDs des blogs
 * -----
 * $Author: Pierre DARGHAM $
 * $Date: 2014/08/06 $
 */
function get_blogs_id() {
     if(!is_multisite()) {
         return array('1');
    }
    ...
    $field = 'blog_id';
    $table = $param['prefix'] .'blogs';

    $sql = 'SELECT '.$field.' FROM '.$table;

    if($rs = mysql_query($sql)) {
        while(($resultArray[] = mysql_fetch_assoc($rs)) || array_pop($resultArray)); 


        foreach($resultArray as $key => $value) {
            $resultArray[$key] = $value['blog_id'];
        }
...
/*
 * Fonction update_blog($blog_id, &$message)
 * -----
 * Met a jour les tables d'un blog donné
 * -----
 * @param   string      $blog_id                l'ID du blog a mettre a jour
 * @param   array       &$message               si une erreur survient, on remplit ce tableau
 * -----
 * $Author: Pierre DARGHAM $
 * $Date: 2014/08/06 $
 */
function update_blog($blog_id, &$message) {
    global $param, $known_plugin;
    ...
    // Les options
    update($prefix.'options', array('option_id', 'option_value'), $message);
    // Les posts
    update($prefix.'posts', array('ID', 'post_content'), $message);
    update($prefix.'posts', array('ID', 'guid'), $message);
    // Les postmetas
    update($prefix.'postmeta', array('meta_id', 'meta_value'), $message);
    // Les liens
    if(!empty($param['link_update'])) {
        update($prefix.'links', array('link_id', 'link_url'), $message);
        update($prefix.'links', array('link_id', 'link_image'), $message);
    }
                ...
                foreach($known_plugin[$name]['update'] as $update) {
                    if(!empty($update['table']) && !empty($update['champ'])) {
                        ...
                        foreach($update['champ'] as $champ)
                            update($prefix.$update['table'], $champ, $message);
                    }
                }
            }
        }
